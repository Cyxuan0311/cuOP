...
template <typename T>
StatusCode AveragePool2D<T>::Forward(const Tensor<T>& input, Tensor<T>& output) {
    ...
    LOG(INFO) << "AveragePool2D launch: grid = (" << grid_size.x << ", " << grid_size.y << "), block = (" << block_size.x << ", " << block_size.y << ")";
    averagepool2D_kernel<T>
